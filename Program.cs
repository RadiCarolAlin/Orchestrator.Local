...
string projectId =
    ...
    ?? "";
string triggerId =
    builder.Configuration["Gcp:TriggerId"]
    ?? Environment.GetEnvironmentVariable("TRIGGER_ID")
    ?? "";
string region =
    builder.Configuration["Gcp:Region"] ?? Environment.GetEnvironmentVariable("REGION") ?? "global";
string progressUrl =
    builder.Configuration["Gcp:ProgressUrl"]
...
static void EnqueueLog(
    ConcurrentDictionary<string, ConcurrentQueue<string>> store,
    string buildId,
    string line
)
{
...
static string NormalizeLive(string s) =>
    (N(s)) switch
    {
        "start" => "RUNNING",
        "done" => "SUCCESS",
...
// store mapping: operationId -> cloudBuildBuildId

ConcurrentDictionary<string, string> operationToBuildId = new();
                ...
                platforms.Add(
                    new
                    {
                        ...
                        deployed_apps = data.ContainsKey("deployed_apps")
                            ? ((List<object>)data["deployed_apps"])
                                .Select(x => x.ToString())
                                .ToArray()
                            : Array.Empty<string>(),
                        user_email = data.ContainsKey("user_email")
...
app.MapPost(
    "/platform/deploy",
    async (DeployPlatformRequest req) =>
    {
        try
        {
            ...
            if (!apps.Contains("frontend") || !apps.Contains("backend"))
            {
                return Results.BadRequest(
                    new { ok = false, error = "Frontend and Backend are required" }
                );
            }
            ...
            var result = await TriggerCloudBuild(
                projectId,
                triggerId,
                region,
                progressUrl,
...
app.MapPost(
    "/platform/add",
    async (ModifyPlatformRequest req) =>
    {
        try
        {
            if (string.IsNullOrWhiteSpace(req.Namespace))
            ...
            if (!snapshot.Exists)
            {
                return Results.BadRequest(
                    ...
                    {
                        ok = false,
                        error = $"Platform '{req.Namespace}' not found. Deploy it first.",
                    }
                );
            }

            var data = snapshot.ToDictionary();
            var currentApps = ((List<object>)data["deployed_apps"])
                .Select(x => x.ToString()!.ToLowerInvariant())
                .ToList();

            var appsToAdd = req
                .Apps.Select(a => a.ToLowerInvariant())
                .Where(a => !currentApps.Contains(a))
                .ToList();

            if (appsToAdd.Count == 0)
            {
                return Results.BadRequest(
                    new { ok = false, error = "All selected apps are already deployed" }
                );
            }
...
app.MapPost(
    ...
    async (ModifyPlatformRequest req) =>
    {
        try
        {
            ...
            {
                return Results.BadRequest(
                    new { ok = false, error = $"Platform '{req.Namespace}' not found" }
                );
            }
            ...
            if (blockedApps.Any())
            {
                return Results.BadRequest(
                    ...
                    {
                        ok = false,
                        error = $"Cannot remove required apps: {string.Join(", ", blockedApps)}",
                    }
                );
            }
            ...
            {
                return Results.BadRequest(
                    new { ok = false, error = "None of the selected apps are currently deployed" }
                );
            }
...
app.MapPost(
    "/platform/delete",
    async (DeletePlatformRequest req) =>
    {
        try
        {
            if (string.IsNullOrWhiteSpace(req.Namespace))
            ...
            var data = snapshot.ToDictionary();
            var allApps = ((List<object>)data["deployed_apps"])
                .Select(x => x.ToString()!.ToLowerInvariant())
                .ToList();
...
)
{
    try
    {
        ...
        var url =
            $"https://cloudbuild.googleapis.com/v1/projects/{projectId}/locations/{region}/triggers/{triggerId}:run";

        var subs = new Dictionary<string, string?> { ["_TARGETS"] = targets, ["_ACTION"] = action };
        ...
        var body = new
        {
            projectId,
            triggerId,
            source = new { branchName = branch, substitutions = subs },
        };
...
// Global mapping: buildId -> operationId (frontend listens on operationId)
ConcurrentDictionary<string, string> BuildToOperationMap = new();

// Called when a new build starts
void RegisterBuild(string buildId, string operationId)
{
    BuildToOperationMap[buildId] = operationId;
}
app.MapPost(
    "/progress",
    async (HttpRequest req, IHubContext<DeployHub> hubContext) =>
    {
        ...
        Console.WriteLine($"📡 Sending to group '{buildId}' with {allLogs.Count} logs");
    ...
    async (HttpContext http, string operation) =>
    {
        try
        {
            ...
            var opPath =
                ...
                : operation.StartsWith("operations/", StringComparison.OrdinalIgnoreCase)
                    ? operation
            ...
            if (done && !string.IsNullOrEmpty(buildId))
            {
                ...
                if (state == "SUCCESS")
                {
                    ...
                    foreach (var platformDoc in platformsSnapshot.Documents)
                    {
                        ...
                        {
                            await platformDoc.Reference.UpdateAsync(
                                new Dictionary<string, object>
                                {
                                    ["status"] = "active",
                                    ["last_modified"] = Timestamp.FromDateTime(DateTime.UtcNow),
...
public record DeployPlatformRequest(
    string[] Apps,
    string? Branch,
    string? Namespace,
    string? UserEmail
